  ...
  const handleCancelSubscription = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: 'Your plan has been changed to the Free Plan.',
      });
    } catch (e) {
      toast({
        variant: 'destructive',
        title: 'Error Cancelling Subscription',
        description: 'There was a problem updating your plan. Please try again.',
      });
      console.error(e);
    }
  };
                ...
                <Card className="bg-muted/50 p-6">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="font-semibold">{format(new Date(userProfile.subscriptionStartDate), 'MMMM dd, yyyy')}</p>
                        </div>
                    </div>
                </Card>
                 <Card className="bg-muted/50 p-6">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Next Renewal Date</p>
        ...
        {isSubscribed ? (
          <div className="w-full">
            ...
            <AlertDialog>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  <AlertDialogTitle>Manage Your Subscription</AlertDialogTitle>
                  <AlertDialogDescription>
                    You are about to cancel your subscription. This will downgrade your account to the Free Plan at the end of your current billing period. Are you sure you want to continue?
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
...
function OverviewTab({ onTabChange }: { onTabChange: (value: string) => void; }) {
  const { user } = useUser();
  const firestore = useFirestore();
    ...
    <div className="space-y-8">
      <div className="grid gap-6 lg:grid-cols-3">
             ...
             {isSubscribed ? (
                <div>
                    <h2 className="text-2xl font-bold">
                        Welcome back, <span className="text-primary">{userProfile?.displayName || 'Explorer'}</span>!
                    </h2>
                ...
                <div>
                    <h2 className="text-2xl font-bold">Unlock Your Ultimate Travel Companion</h2>
                    <p className="mt-1 text-muted-foreground">
                        Upgrade to Wanderwise AI Premium for unlimited itineraries, trip saving, and more.
                    </p>
                </div>
            )}
           </CardHeader>
        ...
        <Card>
            ...
            <CardContent>
                <div className="text-2xl font-bold capitalize">{userProfile?.subscriptionStatus || 'Free'}</div>
                <p className="text-xs text-muted-foreground">
                    {isSubscribed ? 'Thank you for your support!' : 'Upgrade for premium features'}
                </p>
            </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Itineraries Saved</CardTitle>
              <FileText className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">{itineraries?.length || 0}</div>
               <p className="text-xs text-muted-foreground">
                {isSubscribed ? 'Unlimited generations remaining' : `${userProfile?.itinerariesGenerated || 0}/2 used`}
              </p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">AI Messages Sent</CardTitle>
              <MessageCircle className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">{userProfile?.chatMessagesSent || 0}</div>
              <p className="text-xs text-muted-foreground">
                {isSubscribed ? 'Unlimited messages' : `${userProfile?.chatMessagesSent || 0}/1 used`}
              </p>
            </CardContent>
          </Card>
           <Card>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        <Card className="lg:col-span-2">
            <CardHeader>
                ...
                <CardDescription>Your latest adventures are just a click away.</CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <Card>
            ...
            <CardContent className="space-y-4">
                ...
                <Button asChild className="w-full" variant="secondary">
                 ...
                 <Button className="w-full" variant="secondary" onClick={() => onTabChange('profile')}>
